...
test("child control extension proposes fenced growth and uses graph membership", async () => {
  ...
  try {
    ...
    const status: any = await control.execute("e2e", { action: "status" }, new AbortController().signal, undefined, { cwd: fx.root } as any);
    assert.deepEqual(status.details.self, { id: "a", agent: "worker", status: "running", attempt: 1, dependsOn: [], allowedSubagents: ["reviewer"] });
    assert.deepEqual(status.details.nodes.map((node: any) => [node.id, node.status, node.dependsOn]), [["a", "running", []], ["b", "paused", []]]);
    assert.equal(status.details.pendingGrowth, 0);
    assert.equal("task" in status.details.nodes[0], false);
    assert.equal("output" in status.details.nodes[0], false);
    ...
    const checkpoint = JSON.parse(fs.readFileSync(runFile(fx.root, run.id), "utf8")); checkpoint.status = "paused"; checkpoint.revision += 5; checkpoint.nodes[0].status = "succeeded"; atomicWrite(runFile(fx.root, run.id), checkpoint);
