...
test("loads global mesh settings and project overrides", () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(cwd, ".pi", "mesh", "settings.yaml"), "maxConcurrentAgents: 2\nmaxNodes: 20\ndefaultNodeTimeoutMs: 600000\nchildExtensions:\n  mcp: ./mcp.ts\nchildSkills:\n  browser: ./browser/SKILL.md\n");
    assert.deepEqual(loadMeshSettings(cwd, { PI_CODING_AGENT_DIR: agentDir }), { bridge: { enabled: false, routes: [] }, maxAgentDepth: 6, maxConcurrentAgents: 2, maxNodes: 20, defaultNodeTimeoutMs: 600000, messagePayloadMaxBytes: 32768, recipientUnreadMaxBytes: 1048576, childExtensions: { mcp: path.join(cwd, ".pi", "mesh", "mcp.ts") }, childSkills: { browser: path.join(cwd, ".pi", "mesh", "browser", "SKILL.md") }, joinMode: "smart", directCommunication: false, mailboxNotifications: false, debug: true, retentionDays: 30, maxTerminalRuns: 100, debugMaxBytes: 4 * 1024 * 1024 });
    ...
    assert.equal(globalOnly.defaultNodeTimeoutMs, 1800000);
