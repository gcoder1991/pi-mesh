...
export type ContinuationPermit = { bind(runId: string): void; complete(details: object): boolean; claim(callId: string): boolean };
export function requestContinuation(pi: ExtensionAPI, ctx: ExtensionContext, callId: string, input: unknown): ContinuationPermit {
  ...
  open = false;
  if (!permit) throw new Error("Predeclared continuation requires current Cross user-turn authorization; no run created");
  return permit;
}
